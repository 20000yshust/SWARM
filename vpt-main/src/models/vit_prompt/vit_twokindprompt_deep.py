...
class PromptedTransformer(Transformer):
    def __init__(self, prompt_config, config, img_size, vis):
        ...
        if self.prompt_config.INITIATION == "random":
            ...
            # add for backdoor
            # val2 = math.sqrt(6. / float(3 * reduce(mul, patch_size, 1) + prompt_dim))  # noqa
            #
            # self.prompt_embeddings_badone = nn.Parameter(torch.zeros(
            #     1, 1, prompt_dim))
            # # xavier_uniform initialization
            # nn.init.uniform_(self.prompt_embeddings_badone.data, -val2, val2)
    ...
    def incorporate_prompt(self, x):
        ...
        #         # add for backdoor
        #         self.prompt_dropout(self.prompt_proj(self.prompt_embeddings_badone).expand(B, -1, -1)),
        #         x[:, 1:, :]
        #     ), dim=1)
        # elif self.on == False:
