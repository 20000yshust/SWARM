...
from src.engine.trainer import BadVPT
from src.engine.trainer import BadVPT_2stage,BadVPT_2stage_modify2,BadVPT_Scaleup,BadVPT_LIRA,BadVPT_Teco,BadVPT_STRIP,BadVPT_Visualization
from src.engine.trainer import BadVPT_TUAP,BadVPT_DetectionASR,BadVPT_DetectionASR_2
from src.engine.trainer import BadVPT_BadNets
from src.engine.trainer import BadVPT_Blended
from src.engine.trainer import BadVPT_WaNet
from src.engine.trainer import BadVPT_ISSBA,BadVPT_NAD,BadVPT_IBAU
...
def backdoorsetup(args, lr, wd, final_runs, run_idx=None, seed=None):
    """
    Create configs and perform basic setups.
    """
    cfg = get_cfg()
    ...
    if not final_runs:
        ...
        # files = glob.glob(f"{cfg.OUTPUT_DIR}_val/{cfg.DATA.NAME}/{cfg.DATA.FEATURE}/*/run1/eval_results.pth")
        # lr, wd = find_best_lrwd(files, cfg.DATA.NAME)
        cfg.OUTPUT_DIR = cfg.OUTPUT_DIR + "backdoor_final"
        cfg.SOLVER.BASE_LR = lr
        cfg.SOLVER.WEIGHT_DECAY = wd
...
def backdoorsetup_2stage(args, lr, wd, final_runs, run_idx=None, seed=None):
    """
    Create configs and perform basic setups.
    """
    cfg = get_cfg()
    ...
    if not final_runs:
        ...
        # files = glob.glob(f"{cfg.OUTPUT_DIR}_val/{cfg.DATA.NAME}/{cfg.DATA.FEATURE}/*/run1/eval_results.pth")
        # lr, wd = find_best_lrwd(files, cfg.DATA.NAME)
        cfg.OUTPUT_DIR = cfg.OUTPUT_DIR + "backdoor2stage_final"
        cfg.SOLVER.BASE_LR = lr
        cfg.SOLVER.WEIGHT_DECAY = wd
...
def backdoortrain(cfg, args, final_runs):
    # clear up residual cache from previous runs
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    # main training / eval actions here
    ...
    evaluator = Evaluator()
    logger.info("Setting up Trainer...")
    trainer = BadVPT(cfg, model, evaluator, cur_device)
...
def backdoortrain_2stage(cfg, args, final_runs):
    # clear up residual cache from previous runs
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    # main training / eval actions here
    ...
    evaluator = Evaluator()
    logger.info("Setting up Trainer...")
    # trainer = BadVPT_2stage_modify2(cfg, model, evaluator, cur_device)
    # trainer=BadVPT_TUAP(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_BadNets(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_Blended(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_WaNet(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_Scaleup(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_LIRA(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_Teco(cfg, model, evaluator, cur_device)
    # trainer=BadVPT_STRIP(cfg, model, evaluator, cur_device)
    trainer = BadVPT_Visualization(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_ISSBA(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_DetectionASR(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_DetectionASR_2(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_NAD(cfg, model, evaluator, cur_device)
    # trainer = BadVPT_IBAU(cfg, model, evaluator, cur_device)
...
def main(args):
    ...
    for run_idx, seed in enumerate(random_seeds):
        try:
            cfg = setup(
                args, 0.25, 0.01, final_runs=True, run_idx=run_idx+1, seed=seed)
        except ValueError:
            # already ran
            continue
...
def backdoormain(args):
    ...
    for run_idx, seed in enumerate(random_seeds):
        try:
            cfg = backdoorsetup(args, 0.03125, 0.01, final_runs=True, run_idx=run_idx+1, seed=seed)
        except ValueError:
            # already ran
            continue
        backdoortrain(cfg, args, final_runs=True)


def backdoormain_2stage(args):
    """main function to call from workflow"""
    # tuning lr and wd first:
    # lr_range, wd_range = get_lrwd_range(args)
    #
    ...
    #         train(cfg, args, final_runs=False)
    #
    # # final run 5 times with fixed seed0
    # random_seeds = [42, 44, 82, 100, 800]
    random_seeds = [3047]
    for run_idx, seed in enumerate(random_seeds):
        try:
            cfg = backdoorsetup_2stage(args,6.25, 0.001, final_runs=True, run_idx=run_idx+1, seed=seed)
        except ValueError:
            # already ran
            continue
        backdoortrain_2stage(cfg, args, final_runs=True)
