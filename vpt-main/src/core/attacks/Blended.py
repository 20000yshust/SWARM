...
class PoisonedTFDataset(TFDataset):
    ...
    def __getitem__(self, index):
        ...
        # sample: Tensor = torch.from_numpy(np.transpose(sample, (2, 0, 1)))

        sample-=self.img_mean
        sample/=self.img_std
        return sample, target
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index):
    ...
    if class_name == DatasetFolder:
        ...
        return PoisonedTFDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index)
    else:
