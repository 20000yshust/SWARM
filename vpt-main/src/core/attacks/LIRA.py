...
class LIRA(Base):
    ...
    def create_net(self):
        ...
        if self.dataset_name == 'mnist':
            ...
            def create_cifar():
                return VGG('VGG11', num_classes=100) # core.models.vgg11(num_classes=10) #core.models.ResNet(18)
            return create_cifar
    ...
    def train(self, solver,schedule=None):
        ...
        # Optimizer
        # optimizer = torch.optim.SGD(self.model.parameters(), lr=self.current_schedule['lr'], momentum=self.current_schedule['momentum'],weight_decay=0.001)
        tgtoptimizer = torch.optim.SGD(tgtmodel.parameters(), lr=self.current_schedule['lr_atk'])
        optimizer = make_optimizer([self.model], solver)

        self.scheduler = make_scheduler(optimizer, solver)


        self.tmpmodel = deepcopy(self.model).to(self.device)
        tmpoptimizer = make_optimizer([self.tmpmodel], solver)

        self.tmpscheduler = make_scheduler(tmpoptimizer, solver)
        ...
        for epoch in range(start_epoch, self.current_schedule['epochs'] + 1):
            for i in range(self.current_schedule['train_epoch']):
                atkloss, atkcleanloss, atkpoisonloss, atktriloss = self.train_step(atkmodel, tgtmodel, self.model, tmpoptimizer,
                                                                        tgtoptimizer, optimizer, target_transform, train_loader,
                                                                        create_net, clip_image, post_transforms=post_transforms)
                self.scheduler.step()
                self.tmpscheduler.step()
            ...
            scratchmodel = deepcopy(self.model).to(device)
        ...
        self.model = deepcopy(self.model).to(device)
    ...
    def train_step(self, atkmodel, tgtmodel, model,tmpoptimizer,tgtoptimizer, clsoptimizer, target_transform,
        ...
        for batch_idx, input_data in enumerate(train_loader):
            ...
            self.tmpmodel = deepcopy(self.model).to(self.device)
            self.tmpmodel.enc.transformer.on=False
            ...
            # Calculate loss2
            output2=self.tmpmodel(data)
            atkoutput2=self.tmpmodel(atkdata)
            loss_clean2 = self.loss(output2, target)
            loss_poison2 = self.loss(atkoutput2,target_transform(target))
            loss2 = loss_clean2 * self.alpha + (1 - self.alpha) * loss_poison2
            ...
            noise = tgtmodel(data) * self.eps
            atkdata = clip_image(data + noise)
            tgtloss = self.loss(self.tmpmodel(atkdata), target_transform(target))
