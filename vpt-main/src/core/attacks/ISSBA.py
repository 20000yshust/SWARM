...
class GetPoisonedDataset(Dataset):
    """Construct a dataset.
...
class ISSBA(Base):
    ...
    def train(self, schedule=None):
        ...
        torch.save(encoder,"encoder_clevr.pt")

        # cln_train_dataset, cln_train_labset, bd_train_dataset, bd_train_labset = [], [], [], []
        # for idx, input_data in enumerate(train_dl):
        ...
        # for idx, input_data in enumerate(test_dl):
        #     img=input_data["image"].float()
        #     lab=input_data["label"]
        #     img = img.to(self.device)
        #     residual = encoder([secret, img])
        ...
        # torch.save(cln_train_dl,"cln_train_dl_cifar100.pt")
        # torch.save(bd_train_dl,"bd_train_dl_cifar100.pt")
        #
        #
