...
class ResNet(nn.Module):
    ...
    def _setup_prompt_pad(self, prompt_config, model):
        if prompt_config.INITIATION == "random":
            ...
            self.prompt_embeddings_badone = nn.Parameter(torch.zeros(
                1,3,5, self.cfg.DATA.CROPSIZE + 2 * self.num_tokens))
            # xavier_uniform initialization
            nn.init.uniform_(self.prompt_embeddings_badone.data, 0.0, 1.0)
    ...
    def incorporate_prompt(self, x):
        ...
        if self.prompt_location == "below":
            ...
            prompt_emb_bad = self.prompt_norm(
                self.prompt_embeddings_badone).expand(B, -1, -1, -1)
            ...
            if self.on == True:
                self.prompt_embeddings_tb.requires_grad = False
                self.prompt_embeddings_lr.requires_grad = False
                x = torch.cat((
                    prompt_emb_tb[:, :, :self.num_tokens, :],
                    x, prompt_emb_tb[:, :, self.num_tokens:, :],
                    prompt_emb_bad[:, :,:, :]
                ...
                self.prompt_embeddings_tb.requires_grad = True
                self.prompt_embeddings_lr.requires_grad = True
                x = torch.cat((
                    prompt_emb_tb[:, :, :self.num_tokens, :],
                    x, prompt_emb_tb[:, :, self.num_tokens:, :]
                ), dim=-2)
