...
class PoisonedTFDataset(TFDataset):
    def __init__(self,
                 benign_dataset,
                 y_target,
                 is_train_set,
                 ...
                 poisoned_target_transform_index,
                 seed):
        super(PoisonedTFDataset, self).__init__(benign_dataset.cfg,benign_dataset._split)
        random.seed(seed)
    ...
    def gen_poisoned_index(self):
        target_label_list = []
        for (index, t) in enumerate(self._targets):
            if t == self.y_target_ori:
                target_label_list.append(index)
    ...
    def __getitem__(self, index):
        sample, target = self._image_tensor_list[index], int(self._targets[index])
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, mask, poisoned_transform_index,
    ...
    if class_name == DatasetFolder:
        ...
        return PoisonedCIFAR10(benign_dataset, y_target,is_train_set, poisoned_rate, pattern, mask,
                               poisoned_transform_index, poisoned_target_transform_index, seed)
    elif class_name==TFDataset:
        return PoisonedTFDataset(benign_dataset, y_target,is_train_set, poisoned_rate, pattern, mask,
                                     poisoned_transform_index, poisoned_target_transform_index, seed)
...
class UAP:
    def __init__(self, model,  train_dataset, test_dataset, class_name, use_cuda, target_class=0,
        ...
        if class_name == DatasetFolder:
            ...
            self.testloader = torch.utils.data.DataLoader(dataset=self.testset, batch_size=32, pin_memory=True,
                                                          num_workers=0, shuffle=False)
    ...
    def universal_perturbation(self, delta=0.2, max_iter_uni=40, epsilon=10.0/255,
        ...
        while fooling_rate < 1-delta and itr < max_iter_uni:
            ...
            for k in order:
                cur_img= self.trainset[k]["image"].float()  # (3,32,32)
